...
class TrackingController extends Controller
{
    ...
    public function trackingAddToCartOffer(Request $request)
    {
        ...
        try {
            $listProductId = $this->getProductsByShopifyId($triggerId);
            Log::info('$listProductId:'.print_r($listProductId,true));
            $variantProductId = $this->product->where('variant_id', $addedVariant)->first()->id;
            Log::info('$variantProductId:'.print_r($variantProductId,true));
            $listOfferAvailable = $this->getOfferListByProductList($listProductId, $triggerType, $trigger_place);
            Log::info('listofferavailable:'.print_r($listOfferAvailable,true));
            $listTrackingOffer = $this->getListTrackingOffer($listOfferAvailable, $variantProductId);
            Log::info('listtrackingoffer:'.print_r($listTrackingOffer,true));
     ...
     * @return array
     */
    public function getProductsByShopifyId($triggerId)
    {
        $productModel = $this->product;
        $productInternalId = $productModel->where('variant_id',$triggerId)->get();
        $listProduct = [];
